    ...
    def generate_mandate(self, score: RegimeScore, vol: VolMetrics, 
        ...
        if dte <= ProductionConfig.EXIT_DTE:
            return TradingMandate(
                ...
                warnings=[f"⚠️ DTE {dte} - Position exit window only"],
...
class TradingOrchestrator:
    ...
    def execute_best_mandate(self, analysis: Dict) -> Optional[str]:
        ...
        # Use deployment_amount from mandate (already calculated in RegimeEngineV33)
        logger.info(f"Deploying: ₹{mandate.deployment_amount:,.2f} ({mandate.allocation_pct:.1f}%) | Lots: {mandate.max_lots}")
        ...
        # FIX #19: GTT placement - DISABLED for defined-risk spreads
        # GTT logic doesn't match spread mechanics - using RiskMonitor for exits
        gtt_ids = []
        logger.info("GTT orders disabled - using RiskMonitor for exit management")
        ...
        while True:
            try:
                ...
                if TIME_OPEN <= curr_time <= TIME_CLOSE:
                    ...
                    current_30_block = (now_ist.hour, now_ист.minute // 30)
...
def main():
    ...
    db_writer.set_state("system_version", "3.3-PROFESSIONAL-CORRECTED")
    ...
    telegram.send(
        f"🚀 System Startup\n"
        f"Version: 3.3 Professional (CORRECTED)\n"
